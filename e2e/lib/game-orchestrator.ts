...
export class GameOrchestrator {
  ...
  async setupPlayers(count: number = 2): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const actions = new PlayerActions(page, `Player ${i}`);
  ...
  async authenticateAllPlayers(): Promise<void> {
    console.log('Authenticating all players...');
    await Promise.all(
      this.players.map(p => p.actions.authenticateAsGuest())
    );
    console.log('All players authenticated');
  }
  
  async joinGameQueue(): Promise<void> {
    console.log('Players joining game queue...');
    const results = await Promise.all(
      this.players.map(p => p.actions.joinQueue())
    );
  ...
  async waitForGameStart(): Promise<boolean> {
    ...
    const results = await Promise.all(
      this.players.map(p => p.actions.waitForGameStart())
    );
  ...
  async playTurn(turnNumber: number): Promise<void> {
    console.log(`\n=== Turn ${turnNumber} ===`);
    
    // Check for ban phase on all players
    const banPhaseResults = await Promise.all(
  ...
  async captureScreenshots(suffix: string): Promise<void> {
    await Promise.all(
      this.players.map(p => 
        p.actions.captureScreenshot(`player${p.playerNumber}_${suffix}.png`)
      )
    );
  }
  
  async runGameFlow(maxTurns: number = 10): Promise<void> {
