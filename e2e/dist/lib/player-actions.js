...
class PlayerActions {
    ...
    async getCurrentTurn() {
        try {
            ...
            if (pageText?.includes("White's turn") || pageText?.includes("White to move"))
                return 'white';
            if (pageText?.includes("Black's turn") || pageText?.includes("Black to move"))
                return 'black';
            return null;
        }
        catch {
