...
class UsernameFilter {
  ...
  constructor() {
    ...
    // Add more aggressive filters for racial slurs and hate speech
    // Note: bad-words library already includes many inappropriate terms
    ...
    // Create set of reserved usernames for faster lookup
  ...
  /**
   * Check if a username contains profanity (for lighter filtering)
   */
  containsProfanity(username: string): boolean {
    return this.profanityFilter.isProfane(username.toLowerCase().trim());
  }
  
  /**
   * Clean a username by removing profanity (for suggestions)
   */
  cleanUsername(username: string): string {
    return this.profanityFilter.clean(username);
  }
}
