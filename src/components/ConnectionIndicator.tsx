...
const ConnectionIndicator: React.FC<ConnectionIndicatorProps> = ({
  ...
  const [heartbeat, setHeartbeat] = useState<HeartbeatState>({
    lastBeat: 0, // Initialize to 0 to avoid SSR hydration mismatch
    isActive: false,
    roundTripTime: null,
  });
  ...
  // Initialize timestamp on client side to avoid SSR issues
  useEffect(() => {
    setHeartbeat(prev => ({
      ...prev,
      lastBeat: Date.now()
          ...
          <Box sx={{ mt: 2 }}>
            ...
            <Stack spacing={1} sx={{ fontSize: "0.75rem" }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Typography variant="caption" color="text.primary">
                  {heartbeat.lastBeat > 0 ? formatTime(heartbeat.lastBeat) : "..."}
                </Typography>
              </Box>
